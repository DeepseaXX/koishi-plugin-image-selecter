...
插件使用统一的 `userLimits` 列表管理用户上传权限：

1. **配置方式**：在配置页面的 `userLimits` 表格中添加项目。
2. **用户ID (Key)**：填写用户ID。必须包含一个 `userId` 为 `default` 的项作为默认规则。
3. **上传尺寸限制 (Value)**：填写允许上传的最大文件尺寸(MB)。
4. **规则逻辑**：
   - 优先匹配具体用户ID的设置。
   - 若用户ID不在列表中，则使用 `default` 的设置。
   - 若 `default` 也不存在（虽有默认值），或限制值为 `0`，则**禁止上传**。
